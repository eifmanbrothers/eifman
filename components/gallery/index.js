...
const Gallery = () => {
  ...
  const widthHandle = (evt) => {
    ...
    setValue(
      moveValue * ((widthGallery - currentUserWidth) / currentUserWidth)
    );
    setValueFront(
      moveValue * ((widthGalleryFront - currentUserWidth) / currentUserWidth)
    );
  };
  ...
  return (
    <div className={styles.gallery}>
      <ul
        ...
        // onTouchMove={widthHandle}
        // onPointerMove={widthHandle}
        style={{ left: `-${value}px` }}
      >
        {items.slice(7).map((el) => (
          <EventImagine
            id={el.id}
            key={el.id.ru}
            href={el.path}
      ...
      <ul
        className={styles.gallery__list_front}
        ref={containerFront}
        onMouseMove={widthHandle}
        style={{ left: `-${valueFront}px` }}
      >
        {items.slice(0, 7).map((el) => (
          <EventImagine
            id={el.id}
            key={el.id.ru}
            href={el.path}
