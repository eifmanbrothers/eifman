...
class Api {
  ...
  getNewsPart(locale) {
    return fetch(
      `${this.address}/api/news?locale=${locale}&sort=date:desc&pagination[start]=0&pagination[limit]=5`,
      {
  ...
  getNews(locale, page) {
    ...
    return fetch(
      `${
        this.address
      }/api/news?locale=${locale}&populate=image&pagination[pageSize]=${AMOUNT_NEWS}&sort=date:desc&pagination[page]=${
        page || "1"
      }`,
      {
